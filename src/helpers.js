...
export const makeMsgForWrongTiles = (name, numOfTiles) => {
  return `Niepoprawna liczba pól. Statek ${name.toLowerCase()} posiada ${numOfTiles} płytek. Spróbuj jeszcze raz!.`;
};

export const makeMsgForSelectingTiles = (name, numOfTiles) => {
  return `Wybierz ${numOfTiles} pól na ${name.toLowerCase()}.`;
};
...
export const makeMsgForShot = (isMine, ships, coordinate) => {
  ...
  const subject = isMine ? "Ty" : "Przeciwnik";
  const result = isHit ? "Trafiony!" : "Pudło.";
  const { row, column } = coordinate;
  const columnLetter = columnLabel[column];
  const rowNum = row + 1;
  return `${subject} właśnie trafił ${columnLetter}${rowNum}: ${result}`;
};
